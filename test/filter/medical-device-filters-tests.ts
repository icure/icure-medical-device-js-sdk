...
describe('Medical Device Filters Test', function () {
  ...
  it('MedicalDeviceByIds test - Success', async function () {
    const devices = await hcp1Api.medicalDeviceApi.filterMedicalDevices(await new MedicalDeviceFilter().byIds([md1.id!, md2.id!]).build())
  ...
  it('MedicalDeviceByIds test - Failure', async function () {
    const devices = await hcp1Api.medicalDeviceApi.filterMedicalDevices(await new MedicalDeviceFilter().byIds(['DEFINITELY_NON_EXISTING']).build())

    expect(devices.rows.length).to.be.equal(0)
  })

  it('If no parameter is specified, all medical devices are returned', async function () {
    const devices = await hcp1Api.medicalDeviceApi.filterMedicalDevices(await new MedicalDeviceFilter().build())
  ...
  it('Can filter medical devices by union filter', async function () {
    ...
    const otherDevicesFilter = await new MedicalDeviceFilter().byIds([md3.id!, md2.id!]).union([firstDeviceFilter]).build()
  ...
  it('Can filter medical devices by explicit intersection filter', async function () {
    ...
    const devices = await hcp1Api.medicalDeviceApi.filterMedicalDevices(
      await new MedicalDeviceFilter().byIds([md1.id!, md2.id!]).intersection([partialIdFilter]).build()
    )
  ...
  it('Intersection between disjoint sets return empty result', async function () {
    ...
    const devices = await hcp1Api.medicalDeviceApi.filterMedicalDevices(
      await new MedicalDeviceFilter().byIds([md1.id!]).intersection([partialIdFilter]).build()
    )
