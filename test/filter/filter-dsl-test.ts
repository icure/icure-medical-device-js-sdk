...
} from "../../src/filter/patient/PatientByHealthcarePartyDateOfBirthBetweenFilter";
...
} from "../../src/filter/patient/PatientByHealthcarePartyNameContainsFuzzyFilter";
...
describe("Coding Filters Test", function () {
  ...
  it("The same filter can be added more than once with different strategies", async () => {
    ...
    const filter = await new PatientFilter(hcp1Api)
      .forSelf()
      .ofAge(age)
      .dateOfBirthBetween(20221010, 20221010)
      .build()
  ...
  it("Multiple simple filters of the same type can be added", async () => {
    ...
    const filter = await new PatientFilter(hcp1Api)
      .forSelf()
      .containsFuzzy(firstQuery)
      .containsFuzzy(secondQuery)
      .build()
