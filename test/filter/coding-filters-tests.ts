...
describe('Coding Filters Test', function () {
  ...
  it('Can filter Codings by ids', async function () {
    const codes = await hcp1Api.codingApi.filterCoding(await new CodingFilter().byIds([code1.id!, code2.id!]).build())
  ...
  it('Can filter Codings by language', async function () {
    const codes = await hcp1Api.codingApi.filterCoding(await new CodingFilter().byRegionLanguageTypeLabel('be').build())
  ...
  it('Can filter Codings by union filter', async function () {
    const codeByIdFilter = new CodingFilter().byIds([code1.id!])

    const filterByIdOrType = await new CodingFilter().byRegionLanguageTypeLabel('gb', 'en', 'SNOMED').union([codeByIdFilter]).build()
  ...
  it('Can filter Codings by implicit intersection filter', async function () {
    const codes = await hcp1Api.codingApi.filterCoding(
      await new CodingFilter().byRegionLanguageTypeLabel('gb', 'en', 'SNOMED').byIds([code1.id!, code2.id!, code3.id!]).build()
    )
  ...
  it('Can filter Codings by explicit intersection filter', async function () {
    ...
    const codes = await hcp1Api.codingApi.filterCoding(
      await new CodingFilter().byRegionLanguageTypeLabel('gb', 'en', 'SNOMED').intersection([codesByIdFilter]).build()
    )
    expect(codes.rows.length).to.be.equal(1)
    codes.rows.forEach((code) => {
      expect(code.id).to.be.oneOf([code1.id!, code2.id!, code3.id!])
