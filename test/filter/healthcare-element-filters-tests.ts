...
describe('Healthcare Element Filters Test', function () {
  ...
  it('If no parameter is specified, all healthcare Elements for the HCP are returned', async function () {
    ...
    for (const e of elements.rows) {
      const accessInfo = await hcp1Api.cryptoApi.xapi.getDataOwnersWithAccessTo({
        entity: HealthcareElementMapper.toHealthElementDto(e)!,
        type: 'HealthElement',
      })
  ...
  it('Can filter Healthcare Elements by patient', async function () {
    const elements = await hcp1Api.healthcareElementApi.filterHealthcareElement(
      await new HealthcareElementFilter().forDataOwner(hcp1User.healthcarePartyId!).forPatients(hcp1Api.cryptoApi, [patient]).build()
    )
  ...
  it('Can filter Healthcare Elements by code', async function () {
    const elements = await hcp1Api.healthcareElementApi.filterHealthcareElement(
      await new HealthcareElementFilter().forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter('SNOMEDCT', '617').build()
    )
  ...
  it('Can filter Healthcare Elements by tag', async function () {
    const elements = await hcp1Api.healthcareElementApi.filterHealthcareElement(
      await new HealthcareElementFilter().forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter(undefined, undefined, 'SNOMEDCT', '617').build()
    )
  ...
  it('Can filter Healthcare Elements by union filter', async function () {
    const tagFilter = new HealthcareElementFilter().forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter('SNOMEDCT', '617')
