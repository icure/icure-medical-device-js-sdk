...
export namespace FilterMapper {
  ...
  export function toAbstractFilterDto<HealthcareProfessional>(
  ...
  const toUnionFilterCodeDto = (filter: UnionFilter<Coding>) => new UnionFilterDto<CodeDto>(filter.filters.map((it) => toAbstractFilterCodeDto(it)))
  ...
  const toComplementFilterHealthcarePartyDto = (filter: ComplementFilter<HealthcareProfessional>) =>
    new ComplementFilterDto<HealthcarePartyDto>(
  ...
  const toUnionFilterUserDto = (filter: UnionFilter<User>) => new UnionFilterDto<UserDto>(filter.filters.map((it) => toAbstractFilterUserDto(it)))
  ...
  const toComplementFilterMaintenanceTaskDto = (filter: ComplementFilter<Notification>) =>
    new ComplementFilterDto<MaintenanceTaskDto>(
