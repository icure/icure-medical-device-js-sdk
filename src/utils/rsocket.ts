...
>(
  ...
  username: string,
  password: string,
  entityClass: 'Patient' | 'DataSample' | 'User' | 'HealthcareElement' | 'Notification',
  eventTypes: ('CREATE' | 'UPDATE' | 'DELETE')[],
  ...
  const transportWsUrl = `${basePath.startsWith('https:') ? 'wss' : 'ws'}://${basePath.replace(
    /https?:\/\/([^\/]+)(\/rest\/v[123])?\/?/,
    '$1'
  )}/rsocket`
  ...
  const requestStreamFlowable = <O>(
    socket: ICureRSocket<unknown, string | Buffer | Uint8Array>,
    eventTypes: ('CREATE' | 'UPDATE' | 'DELETE')[],
    entityClass: 'Patient' | 'DataSample' | 'User' | 'HealthcareElement' | 'Notification',
    filter: Filter<O> | undefined,
    auth: Buffer
...
export class ICureRSocket<D, M> implements ReactiveSocket<D, M> {
  ...
  constructor(clientFactory: () => RSocketClient<D, M>, reject: (reason?: any) => void = () => {}) {
  ...
  private async reconnectWithRetry(maxRetry: number = 5, retryIntervalMs: number = 10000, exponentialFactor: number = 1.8) {
    this._clientFactory()
      .connect()
      .then(
        (socket) => {
